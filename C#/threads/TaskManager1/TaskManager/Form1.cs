...
namespace TaskManager
{
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			Thread t = new Thread(this.ReloadingThread);
			t.Start();
		}
		...
		private void LoadItems()
		{
			...
			foreach(Process p in processes)
				listView1.Items.Add(this.createLVI(p));
		...
		private void ReloadItems()
		{
			lock (this)
			{
				...
				foreach (Process p in processes)
				{
					...
					{
						if ((int)lvi.SubItems[0].Tag == p.Id)
						{
							q = true;
							if (p.ProcessName != lvi.SubItems[0].Text)
								lvi.SubItems[0].Text = p.ProcessName;

							if (p.MachineName != ".")
								lvi.SubItems[1].Text = p.MachineName;

							try
							{
								if (lvi.SubItems[2].Text != "Unknown")
									lvi.SubItems[2].Text = p.TotalProcessorTime.ToString();
							}
							catch
							{
							...
							lvi.SubItems[4].Text = Form1.BytesToString(p.PagedMemorySize64);
						}
					}
					if (!q)
						listView1.Items.Add(this.createLVI(p));
		...
		private void ReloadingThread()
		{
			Thread.Sleep(100);
			reload r = ReloadItems;
			while (true)
			{
				Thread.Sleep(1000);
				listView1.Invoke(r);
		...
		private ListViewItem createLVI(Process p)
		{
			ListViewItem lvi = new ListViewItem(p.ProcessName);
			lvi.SubItems[0].Tag = p.Id;
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			try
			{
				Process.GetProcessById((int)listView1.SelectedItems[0].SubItems[0].Tag).Kill();
			}
			catch
			{
		...
		private void listView1_MouseClick(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Right)
			{
				if (listView1.FocusedItem.Bounds.Contains(e.Location) == true)
				{
					...
					try
					{
						ProcessModuleCollection pmc = Process.GetProcessById((int)listView1.SelectedItems[0].SubItems[0].Tag).Modules;
					}
					catch
					{
					...
					try
					{
						ProcessThreadCollection ptc = Process.GetProcessById((int)listView1.SelectedItems[0].SubItems[0].Tag).Threads;
					}
					catch
					{
					...
					try
					{
						ProcessPriorityClass priority = Process.GetProcessById((int)listView1.SelectedItems[0].SubItems[0].Tag).PriorityClass;
		...
		private void ToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				process = Process.GetProcessById((int)listView1.SelectedItems[0].SubItems[0].Tag);
